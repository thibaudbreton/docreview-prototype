...
- **Screen 2's Q&A register and this screen's Issuer-responses list are two views onto the same underlying question lifecycle, not two independent datasets.** Both read and write the same in-memory `QA` array: a question drafted and sent from Follow-up's register genuinely appears here once sent, and recording its answer here genuinely unblocks the branch back on Follow-up's table. The two screens differ in audience and stage (internal drafting/batching vs. official issuer-answer recording), not in data ownership.
- **An issuer answer unblocks a branch via two different match paths** — either the branch's own `qaRef` points at the answered question, or the branch's parent requirement id appears in the question's `reqs` list. Both paths are checked; a branch matching either is unblocked.
- **A matched/unblocked branch is not itself "answered."** The issuer's reply closes the *question*, not the *requirement* — the branch returns to `awaiting_answer`, still needing the expert's actual verdict, consistent with the domain model's rule that a Q&A answer removes a blocker rather than substituting for the expert's judgment.
