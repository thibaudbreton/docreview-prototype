...
- [x] **TH1 — DONE.** Reduced the bottom padding on both files' `.wrap` from the hardcoded `80px` (copied from the scroll containers where it's load-bearing against the floating bulk-action bar, but dead space here — neither screen has any floating element covering its bottom edge) to `var(--space-8)` (32px), per the ticket's suggested fix. `accueil.html`: `padding:var(--space-8) var(--space-6) 80px` → `padding:var(--space-8) var(--space-6) var(--space-8)`. `dashboard-et-config.html`: `padding:var(--space-6) var(--space-6) 80px` → `padding:var(--space-6) var(--space-6) var(--space-8)`. `build_merge.py` re-run clean (7/7 screen blobs decode as UTF-8, 0 dead `href="*.html"` links in the merged output); no `<script>` blocks were touched by this change.
