...
- **2026-09-16 (nightly routine):** Completed: TH1 (drop unjustified 80px bottom padding on Home/Dashboard `.wrap`). Blocked: none. Remaining untouched: TH2, TH3, TH4, TH5, TH6.
...
- [x] **TH1 — DONE.** Replaced the hardcoded `80px` bottom value in both files' `.wrap` with `var(--space-8)` (32px) — neither screen has a floating bottom element to clear, so the large fixed padding was pure dead space copied from the document-review screens where it's load-bearing. `accueil.html`: `padding:var(--space-8) var(--space-6) 80px` → `padding:var(--space-8) var(--space-6) var(--space-8)`. `dashboard-et-config.html`: `padding:var(--space-6) var(--space-6) 80px` → `padding:var(--space-6) var(--space-6) var(--space-8)`. Verified: `build_merge.py` rebuild clean, both screens' script blocks pass `node --check`, all 7 source blobs decode as UTF-8, 0 dead `href="*.html"` links in the merged output.

  Original ticket text, for reference:
  **Drop the unjustified 80px bottom padding on Home and Dashboard**
