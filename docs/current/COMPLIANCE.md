...
| CONF-024 | DEC-080 : **ajustements du panneau de décision.** Le verdict se **choisit d'abord** (Compliant / Not compliant), puis seuls les champs de ce choix s'affichent, avec **un seul bouton de confirmation** — plus de bouton Compliant à côté d'un Not compliant ouvert (Compliant passe donc à deux gestes, choisir puis confirmer). **Ask the client** ouvre un champ pour écrire la question ; le contributeur garde la main et peut décider sans attendre la réponse. Sous le texte, **View in the document** bascule sur la vue Document à cette exigence sans quitter le panneau. L'onglet de ressources Document devient **Q&A** : nos questions sur l'exigence et les réponses publiées par le client aux questions des autres soumissionnaires. Un titre « Requirement » coiffe le texte ; la ligne « Yours · … » disparaît ; Set aside et l'élargissement sont des boutons libellés. **Précise CONF-023.** |
| CONF-025 | DEC-081 : **deuxième revue de Compliance.** Table : flèche des sous-lignes placée comme dans Allocation (dans la cellule du texte) ; plus d'interrupteur « By section / By contributor » ni de raccourcis affichés ; jauge réduite à un anneau et un compte ; **export avec options** (format Excel / CSV / PDF, lignes : tout / vue courante / sélection, colonnes au choix, langue du texte) ; **colonnes personnalisées** propres à Compliance ; **Set aside** retrouvable par une pastille-filtre et visible par une cellule teintée (violet, personnel). Panneau de décision : Set aside et élargir au-dessus de l'ID ; texte sur fond teinté avec un interrupteur **Original · <langue>** quand le tender n'est pas en anglais ; choix Compliant / Not compliant de nouveau en vert et rouge ; Ask the client et Not mine au style des boutons de réassignation d'Allocation ; fiches Q&A sur une colonne. |
| CONF-026 | DEC-082 : **le verdict d'un partenaire externe est saisi par le chef de projet**, au niveau système, depuis ce que le partenaire a renvoyé (e-mail, Excel, appel) ; même flux choisir puis confirmer, Not compliant exige Category + Topic. Il consolide comme tout autre système — pas d'exemption, pas de nouveau statut. La réponse enregistrée dit qu'elle a été saisie par le chef de projet pour le partenaire. L'export « What the filters show » annonce le nombre de lignes et le filtre actif en clair. |
| CONF-027 | DEC-083 : **le tri de Compliance se fait par les en-têtes**, comme dans Allocation ; la liste « Sort: … » disparaît, et avec elle l'ordre « action needed first ». Sans tri, la table est dans l'ordre du document, avec ses titres. |
