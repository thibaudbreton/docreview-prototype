...
- **ABS / PBS / OBS** — the three assignment dimensions (activity / product / organisation breakdown structure) an allocation is derived from. **Correction (2026-09-04) — `TICKET-two-pass-allocation.md`:** not three equal inputs to one derivation. The chain is sequential — **PBS → ABS → OBS**, each step derived from the one before it — never a crossing. On a **Turnkey** tender a distribution pass runs first: characterisation picks a single dimension (non-technical reads the ABS, technical reads the PBS) to produce a **TK OBS**, which is an *activity* and routes the requirement into that activity's own model. Every other tender type skips this pass — it is already scoped to one activity. Inside an activity's own model, OBS is a *team or service* (what casting staffs), not the same thing the TK OBS names — see `SPEC-domain-model.md`'s two-pass section for the full model.
